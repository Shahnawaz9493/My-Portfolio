<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&height=280&section=header&text=Hi,%20I'm%20Mohammad%20Shahnawaz&fontSize=50&animation=fadeIn&fontAlignY=38&desc=Developer%20|%20AI%20Enthusiast%20|%20AI%20Agent%20Builder&descAlignY=58" width="100%"/>
</p>

<div align="center">

[![Typing SVG](https://readme-typing-svg.demolab.com?font=Fira+Code&size=18&pause=1000&color=2DE370&background=0D111700&center=true&width=450&lines=Probably+building+AI+agents+right+now)](https://git.io/typing-svg)

<p align="center">
  🎓 Computer Science Engineering Student | 🤖 AI Enthusiast | 💻 Developer | 🤖 AI Agent Builder
</p>

<p align="center">
  📍 CMR Engineering College | 📊 CGPA: 8.47 <br>
  📧 shanu.sufiyan.2805@gmail.com <br>
  🔗 <a href="https://linkedin.com/in/mohammad-shahnawaz">LinkedIn</a> • 💻 <a href="https://github.com/Shahnawaz9493">GitHub</a>
</p>

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=Shahnawaz9493&label=Profile%20Views&color=blueviolet&style=for-the-badge" />
</p>

</div>

---

<div align="center">

### 🔥 Connect With Me

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/mohammad-shahnawaz)
[![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/Shahnawaz9493)
[![Gmail](https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:shanu.sufiyan.2805@gmail.com)

</div>

---

## 🚀 About Me

```python
class Developer:
    def __init__(self):
        self.name = "Mohammad Shahnawaz"
        self.role = "CSE Student | AI Enthusiast | Developer"
        self.college = "CMR Engineering College"
        self.cgpa = 8.47
        self.interests = [
            "Machine Learning 🤖",
            "Data Analytics 📊",
            "Web Development 💻",
            "AI Agent Development 🤖"
        ]
        self.looking_for = "Internship opportunities in AI/ML"
    
    def currently_learning(self):
        return ["AI Agent Builder", "LLM Integration", "Gemini AI API"]
    
    def inspiration(self):
        return "Building AI that builds the future 🚀"
```

---

<h2 align="center">💻 Tech Stack</h2>

<div align="center">

### 🧑‍💻 Languages
![Java](https://img.shields.io/badge/Java-ED8B00?style=for-the-badge&logo=openjdk&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black)
![TypeScript](https://img.shields.io/badge/TypeScript-007ACC?style=for-the-badge&logo=typescript&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![C](https://img.shields.io/badge/C-00599C?style=for-the-badge&logo=c&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-4479A1?style=for-the-badge&logo=postgresql&logoColor=white)

### 🎨 Frontend
![React](https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react&logoColor=61DAFB)
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white)

### ⚙️ Backend & AI
![Node.js](https://img.shields.io/badge/Node.js-6DA55F?style=for-the-badge&logo=node.js&logoColor=white)
![Express.js](https://img.shields.io/badge/Express.js-404D59?style=for-the-badge&logo=express&logoColor=white)
![n8n](https://img.shields.io/badge/n8n-black?style=for-the-badge&logo=n8n&logoColor=white)
![Google Gemini](https://img.shields.io/badge/Gemini-Google-8AB4F8?style=for-the-badge&logo=google)

### 🗄️ Databases & Cloud
![MongoDB](https://img.shields.io/badge/MongoDB-4EA94B?style=for-the-badge&logo=mongodb&logoColor=white)
![MySQL](https://img.shields.io/badge/MySQL-00000F?style=for-the-badge&logo=mysql&logoColor=white)
![AWS](https://img.shields.io/badge/AWS-FF9900?style=for-the-badge&logo=amazon-aws&logoColor=white)
![Cloudinary](https://img.shields.io/badge/Cloudinary-3448C5?style=for-the-badge&logo=cloudinary&logoColor=white)

### 🛠️ Tools & Platforms
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white)
![Netlify](https://img.shields.io/badge/Netlify-00C7B7?style=for-the-badge&logo=netlify&logoColor=white)
![Postman](https://img.shields.io/badge/Postman-FF6C37?style=for-the-badge&logo=postman&logoColor=white)
![Jest](https://img.shields.io/badge/Jest-C21325?style=for-the-badge&logo=jest&logoColor=white)
![Apify](https://img.shields.io/badge/Apify-2D2D2D?style=for-the-badge&logo=apify&logoColor=white)

</div>

---

<div align="center">

### 🤖 Machine Learning & AI
![Logistic Regression](https://img.shields.io/badge/-Logistic%20Regression-blue?style=flat) • ![Random Forest](https://img.shields.io/badge/-Random%20Forest-green?style=flat) • ![XGBoost](https://img.shields.io/badge/-XGBoost-orange?style=flat) • ![AI Agents](https://img.shields.io/badge/-AI%20Agent-purple?style=flat) • ![NLP](https://img.shields.io/badge/-NLP-red?style=flat)

### 📊 Data Analysis
![Data Visualization](https://img.shields.io/badge/-Data%20Viz-blue?style=flat) • ![Dashboards](https://img.shields.io/badge/-Dashboards-green?style=flat) • ![Model Evaluation](https://img.shields.io/badge/-Model%20Eval-orange?style=flat)

### ⚡ AI & Automation
![AI Agent Builder](https://img.shields.io/badge/-AI%20Agent%20Builder-purple?style=flat) • ![API Integration](https://img.shields.io/badge/-API%20Integration-blue?style=flat) • ![Gemini AI](https://img.shields.io/badge/-Gemini%20AI-green?style=flat)

</div>

---

## 📂 Featured Projects

### 🎨 AI-Powered Infographic Creator ⭐
<div align="center">

![n8n](https://img.shields.io/badge/n8n-000000?style=for-the-badge&logo=n8n&logoColor=white)
![Gemini](https://img.shields.io/badge/Gemini-Google-8AB4F8?style=for-the-badge&logo=google)
![Cloudinary](https://img.shields.io/badge/Cloudinary-3448C5?style=for-the-badge&logo=cloudinary)
![Apify](https://img.shields.io/badge/Apify-2D2D2D?style=for-the-badge&logo=apify)

</div>

> 💡 **Transform any YouTube video into a visually stunning infographic — automatically!**

- ✅ Built a fully automated n8n workflow that converts YouTube videos to infographics
- ✅ Integrated **Apify** for automatic transcript extraction
- ✅ Used **Google Gemini AI** for content analysis and image generation
- ✅ Deployed **Cloudinary** for instant image hosting and CDN
- ✅ Zero-code solution — just import and run

| Component | Technology |
|:---|:---|
| Video Transcript | Apify API |
| AI Content Analysis | Google Gemini |
| Image Generation | Gemini Vision |
| Cloud Hosting | Cloudinary |
| Workflow Engine | n8n |

**Flow:** 📝 Submit URL → 📜 Get Transcript → 🧠 AI Summarizes → 🎨 Generate → ☁️ Get Link

**Status:** ✅ Live | **Link:** 🔗 [View Project](https://github.com/Shahnawaz9493/Infographic_creator)

---

### 😊 Human Emotion Recognition System
<div align="center">

![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![TensorFlow](https://img.shields.io/badge/TensorFlow-FF6F00?style=for-the-badge&logo=tensorflow&logoColor=white)
![OpenCV](https://img.shields.io/badge/OpenCV-5C3EE8?style=for-the-badge&logo=opencv&logoColor=white)

</div>

> 🔍 AI-powered facial emotion detection system that recognizes human emotions in real-time.

- ✅ Built an **AI-based deep learning model** for facial emotion detection
- ✅ Used **OpenCV** for image preprocessing and face detection
- ✅ Detects **6 emotions**: Happy, Sad, Angry, Surprise, Fear, Neutral
- ✅ Applied **feature engineering** and data augmentation
- ✅ Evaluated using **accuracy metrics and confusion matrix**

| Component | Technology |
|:---|:---|
| Model | CNN (Convolutional Neural Network) |
| Image Processing | OpenCV |
| Deep Learning | TensorFlow/Keras |
| Dataset | FER2013 |

**Accuracy:** 85%+ | **Link:** 🔗 [View Project](https://github.com/Shahnawaz9493/Emotion_Detector)

---

### 🏙️ Civic Pulse — Smart Civic Issue Platform
<div align="center">

![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black)
![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white)

</div>

> 🏢 A platform that bridges the gap between citizens and government for reporting and tracking civic issues.

- ✅ Platform for **reporting and tracking civic issues**
- ✅ **Database-integrated** complaint management system
- ✅ Real-time **status tracking** for submitted issues
- ✅ Responsive design for **mobile and desktop**

| Feature | Implementation |
|:---|:---|
| Frontend | HTML5, CSS3, JavaScript |
| Database | MySQL |
| Auth | User Login/Registration |

**Status:** ✅ Deployed | **Link:** 🔗 [View Project](https://github.com/Shahnawaz9493/civic_pulse)

---

### 🏥 Sahayak — AI Health Chatbot 🥈
<div align="center">

![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![NLP](https://img.shields.io/badge/NLP-brightgreen?style=for-the-badge&logo=brain)
![Hackathon](https://img.shields.io/badge/-2nd%20Prize-orange?style=for-the-badge)

</div>

> 🤖 AI-powered healthcare chatbot that provides basic health guidance and symptom-based recommendations.

- ✅ Built during **National Level Hackathon** — 🥈 **2nd Prize Winner**
- ✅ **AI chatbot** for basic healthcare guidance
- ✅ Designed **conversational logic** using NLP
- ✅ Provides **instant symptom analysis** and recommendations

| Feature | Technology |
|:---|:---|
| Language Processing | NLP |
| Intent Recognition | Pattern Matching |
| Response Generation | Rule-Based AI |

**Achievement:** 🥈 2nd Prize in Hackathon

---

### 🛍️ Krish's Handicraft Store
<div align="center">

![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black)

</div>

> 🛒 A responsive e-commerce frontend website for a handicraft store with clean UI and interactive features.

- ✅ **Responsive frontend** e-commerce website
- ✅ **Clean UI** with modern design
- ✅ **Mobile-first** design approach
- ✅ Product gallery with **hover effects**

**Status:** ✅ Complete

---

### 🖐️ Gesture-Controlled Desktop Interface
<div align="center">

![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![OpenCV](https://img.shields.io/badge/OpenCV-5C3EE8?style=for-the-badge&logo=opencv&logoColor=white)
![MediaPipe](https://img.shields.io/badge/MediaPipe-00B4AB?style=for-the-badge&logo=google&logoColor=white)

</div>

> 👋 A real-time gesture-based system to control desktop functions using hand movements.

- ✅ **Real-time hand tracking** using MediaPipe and OpenCV
- ✅ **Gesture recognition** for multiple desktop controls
- ✅ Enabled: Cursor movement, Click, Drag, Volume, Media playback
- ✅ **Custom HUD** for real-time feedback

| Gesture | Action |
|:---|:---|
| ✋ Open Palm | Move Cursor |
| 👍 Thumb Up | Left Click |
| ✌️ Peace Sign | Right Click |
| ✊ Fist | Drag Mode |
| 👆 Index Up | Volume Control |

**Tech:** Python, OpenCV, MediaPipe, PyAutoGUI, Pycaw

---

### 🎙️ Voice Command Assistant
<div align="center">

![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![Speech](https://img.shields.io/badge/Speech-Recognition-FF6F00?style=for-the-badge&logo=microphone)
![TTS](https://img.shields.io/badge/TTS-blue?style=for-the-badge&logo=speaker)

</div>

> 🎤 Voice-controlled system automation tool that executes commands using speech recognition.

- ✅ **Voice-controlled** system automation tool
- ✅ Executes: Open apps, Web searches, System controls
- ✅ **SpeechRecognition** for accurate voice input
- ✅ **Pyttsx3** for text-to-speech responses

| Feature | Implementation |
|:---|:---|
| Voice Input | SpeechRecognition |
| Voice Output | Pyttsx3 |
| Control | OS & Web Automation |

**Status:** 🚧 Under Development

---

## 🏆 Achievements & Certifications

<div align="center">

### 🥇 Awards
| Award | Details |
|:---|:---|
| 🥈 | **2nd Prize** — Hackathon (Sahayak AI Chatbot) |
| 🧩 | **Active Participant** — Multiple National Level Hackathons |
| 🎉 | **Tech Fest Lead** — College coding competitions |
| ⭐ | **Top Contributor** — AI & Automation Projects |

### 📜 Professional Certifications
| Organization | Certificate | Verification |
| :--- | :--- | :--- |
| **Microsoft** | AI SKILLS PASSPORT | [View Certificate](https://github.com/Shahnawaz9493/Shahnawaz9493/blob/main/AI%20Skills%20Passport.pdf) |
| **Salesforce** | SALESFORCE AGENTFORCE SPECIALIST | [View Certificate](https://github.com/Shahnawaz9493/Shahnawaz9493/blob/main/Salesforce%20AgentForce%20Specialist.pdf) |
| **Spoken Tutorial** | RDBMS | [View Certificate](https://github.com/Shahnawaz9493/Shahnawaz9493/blob/main/RDMS.pdf) |
| **Spoken Tutorial** | PERL | [View Certificate](https://github.com/Shahnawaz9493/Shahnawaz9493/blob/main/PERL.pdf) |
| **Deloitte** | CYBER JOB SIMULATION | [View Certificate](https://github.com/Shahnawaz9493/Shahnawaz9493/blob/main/CYBER%20JOB%20SIMULATION.pdf) |
| **Hexart.in** | AI FOUNDATION | [View Certificate](https://github.com/Shahnawaz9493/Shahnawaz9493/blob/main/HEXART.AI%20AI%20FOUNDATION.pdf) |
| **Infosys** | CYBER SECURITY | [View Certificate](https://github.com/Shahnawaz9493/Shahnawaz9493/blob/main/Infosys%20Cyber%20Security.pdf) |
| **Infosys** | NETWORK FUNDAMENTALS | [View Certificate](https://github.com/Shahnawaz9493/Shahnawaz9493/blob/main/Infosys%20Network%20Fundamentals.pdf) |
| **MongoDB** | MONGODB SKILLS | [View Certificate](https://github.com/Shahnawaz9493/Shahnawaz9493/blob/main/MongoDB%20Skill.pdf) |
| **SkillUp** | GENERATIVE AI FOR BEGINNERS | [View Certificate](https://github.com/Shahnawaz9493/Shahnawaz9493/blob/main/SkillUp.pdf) |

</div>

---

## 📊 GitHub Stats

<div align="center">

![Streak](https://github-readme-streak-stats.herokuapp.com/?user=Shahnawaz9493&theme=radical&hide_border=true)

</div>

---

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=Shahnawaz9493&label=Profile%20Views&color=blueviolet&style=for-the-badge" />
</p>

> 💭 *"Code today. Innovate tomorrow. Build AI that builds the future."*

---

<div align="center">

### 👇 Let's Connect!

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/mohammad-shahnawaz)
[![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/Shahnawaz9493)
[![Gmail](https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:shanu.sufiyan.2805@gmail.com)

---

⭐️ From [Shahnawaz9493](https://github.com/Shahnawaz9493)

</div>
